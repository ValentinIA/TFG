...
async def obtener_productos_amazon(producto: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener productos de Amazon: {str(e)}")
...
async def obtener_productos_fnac(producto: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener productos de Fnac: {str(e)}")
...
async def obtener_productos_mediamarkt(producto: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al obtener productos de MediaMarkt: {str(e)}")

@router.get("/todos/{producto}")
async def buscar_en_todas_tiendas(producto: str):
